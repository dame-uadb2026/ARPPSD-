...
La base est hébergée sur Supabase et accessible via DATABASE_URL.
"""
import os
import uuid
import json
...
def _dsn():
    ...
    if not dsn:
        raise RuntimeError(
            "La variable DATABASE_URL n'est pas définie. Ajoute-la sur Render "
...
def get_db():
    if "db" not in g:
        g.db = psycopg2.connect(_dsn(), cursor_factory=psycopg2.extras.RealDictCursor)
...
SCHEMA_STATEMENTS = [
    ...
    """CREATE TABLE IF NOT EXISTS caisse_operation (
        ...
        date TEXT, motif TEXT, responsable TEXT, is_demo INTEGER DEFAULT 0)""",
    """CREATE TABLE IF NOT EXISTS activity (
        id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT,
        date TEXT, time TEXT, location TEXT, responsable TEXT,
        status TEXT DEFAULT 'prévue', report TEXT, is_demo INTEGER DEFAULT 0)""",
    ...
    """CREATE TABLE IF NOT EXISTS announcement (
        id TEXT PRIMARY KEY, title TEXT NOT NULL, content TEXT, date TEXT,
        author TEXT, status TEXT DEFAULT 'publié', is_demo INTEGER DEFAULT 0)""",
    """CREATE TABLE IF NOT EXISTS document (
        id TEXT PRIMARY KEY, name TEXT NOT NULL, category TEXT, note TEXT,
        date_added TEXT, added_by TEXT, is_demo INTEGER DEFAULT 0,
        filename TEXT, original_name TEXT, file_size INTEGER, drive_file_id TEXT)""",
    """CREATE TABLE IF NOT EXISTS audit_log (
        id TEXT PRIMARY KEY, action TEXT, "user" TEXT, timestamp TEXT, details TEXT)""",
    """CREATE TABLE IF NOT EXISTS setting (
        key TEXT PRIMARY KEY, value TEXT)""",
    """CREATE TABLE IF NOT EXISTS notification (
        id TEXT PRIMARY KEY, title TEXT NOT NULL, message TEXT, link TEXT,
        target_role TEXT, target_member_id TEXT, created_at TEXT)""",
    """CREATE TABLE IF NOT EXISTS notification_read (
        notification_id TEXT NOT NULL REFERENCES notification(id) ON DELETE CASCADE,
        member_id TEXT NOT NULL, PRIMARY KEY (notification_id, member_id))""",
]
...
def init_db(app):
    with app.app_context():
        ...
        for stmt in SCHEMA_STATEMENTS:
            cur.execute(stmt)
        db.commit()
        _ensure_columns("member", [("password_hash", "password_hash TEXT")])
        _ensure_columns("document", [
...
def get_setting(key, default=None):
    cur = _execute("SELECT value FROM setting WHERE key = ?", (key,))
    row = cur.fetchone()
    if row is None:
        return default
    try:
...
def get_members(status=None):
    if status and status != "tous":
        rows = _execute("SELECT * FROM member WHERE status = ? ORDER BY first_name", (status,)).fetchall()
    else:
        rows = _execute("SELECT * FROM member ORDER BY first_name").fetchall()
    return [row_to_member(r) for r in rows]


def get_member(member_id):
    return row_to_member(_execute("SELECT * FROM member WHERE id = ?", (member_id,)).fetchone())


def count_members():
    return _execute("SELECT COUNT(*) c FROM member").fetchone()["c"]
...
def toggle_member_status(member_id):
    ...
    _execute("UPDATE member SET status=? WHERE id=?", (new_status, member_id))
    _commit()
    return new_status


def set_member_password(member_id, password):
    _execute("UPDATE member SET password_hash=? WHERE id=?",
             (generate_password_hash(password), member_id))
    _commit()


def verify_member_password(member_id, password):
    row = _execute("SELECT password_hash FROM member WHERE id=?", (member_id,)).fetchone()
...
def count_contributions_for_member(member_id, year):
    return _execute("SELECT COUNT(*) c FROM contribution WHERE member_id=? AND year=?",
                    (member_id, year)).fetchone()["c"]
...
def sum_contributions(year=None, month=None):
    ...
    return _execute(q, tuple(params)).fetchone()["s"]


def get_contribution(contrib_id):
    return row_to_contribution(
        _execute("SELECT * FROM contribution WHERE id=?", (contrib_id,)).fetchone())


def find_contribution(member_id, month, year):
    return row_to_contribution(_execute(
        "SELECT * FROM contribution WHERE member_id=? AND month=? AND year=?",
...
def upsert_contribution(member_id, month, year, amount, date_paid, recorded_by):
    ...
    if existing:
        _execute("UPDATE contribution SET amount=?, date_paid=?, recorded_by=? WHERE id=?",
...
def delete_contribution(contrib_id):
    _execute("DELETE FROM contribution WHERE id=?", (contrib_id,))
    _commit()
...
def get_caisse_ops(type_filter=None):
    if type_filter and type_filter != "tous":
        rows = _execute("SELECT * FROM caisse_operation WHERE type=? ORDER BY date DESC",
                        (type_filter,)).fetchall()
    else:
        rows = _execute("SELECT * FROM caisse_operation ORDER BY date DESC").fetchall()
    return [row_to_caisse(r) for r in rows]


def caisse_totals():
    total_in = _execute("SELECT COALESCE(SUM(amount),0) s FROM caisse_operation WHERE type='entrée'").fetchone()["s"]
    total_out = _execute("SELECT COALESCE(SUM(amount),0) s FROM caisse_operation WHERE type='sortie'").fetchone()["s"]
    return total_in, total_out, total_in - total_out


def insert_caisse_op(type_, amount, date_, motif, responsable):
    _execute(
        "INSERT INTO caisse_operation (id, type, amount, date, motif, responsable, is_demo) "
        "VALUES (?,?,?,?,?,?,0)",
...
def _attach_participants(activity):
    rows = _execute(
        ...
        "WHERE ap.activity_id = ? ORDER BY m.first_name", (activity.id,)).fetchall()
...
def get_activities(status_filter=None):
    if status_filter and status_filter != "tous":
        rows = _execute("SELECT * FROM activity WHERE status=? ORDER BY date DESC",
                        (status_filter,)).fetchall()
    else:
        rows = _execute("SELECT * FROM activity ORDER BY date DESC").fetchall()
...
def get_activity(activity_id):
    a = row_to_activity(_execute("SELECT * FROM activity WHERE id=?", (activity_id,)).fetchone())
    if a:
        _attach_participants(a)
    return a
...
def count_activities(status=None):
    if status:
        return _execute("SELECT COUNT(*) c FROM activity WHERE status=?", (status,)).fetchone()["c"]
    return _execute("SELECT COUNT(*) c FROM activity").fetchone()["c"]
...
def insert_activity(data, participant_ids):
    ...
    _execute(
        ...
        (aid, data["title"], data.get("description"), data["date"], data.get("time"),
         data.get("location"), data.get("responsable"), data.get("status", "prévue"),
         data.get("report")))
    for mid in participant_ids:
        _execute("INSERT INTO activity_participant (activity_id, member_id) VALUES (?,?) "
                 "ON CONFLICT DO NOTHING", (aid, mid))
    _commit()
    return aid
...
def update_activity(activity_id, data, participant_ids):
    _execute(
        ...
        (data["title"], data.get("description"), data["date"], data.get("time"),
         data.get("location"), data.get("responsable"), data.get("status", "prévue"),
         data.get("report"), activity_id))
    _execute("DELETE FROM activity_participant WHERE activity_id=?", (activity_id,))
    for mid in participant_ids:
        _execute("INSERT INTO activity_participant (activity_id, member_id) VALUES (?,?) "
                 "ON CONFLICT DO NOTHING", (activity_id, mid))
    _commit()


def get_upcoming_activities(limit=3):
    rows = _execute(
        "SELECT * FROM activity WHERE status IN ('prévue','en cours') ORDER BY date ASC LIMIT ?",
        (limit,)).fetchall()
...
def get_announcements():
    rows = _execute("SELECT * FROM announcement ORDER BY date DESC").fetchall()
    return [row_to_announcement(r) for r in rows]


def get_recent_published_announcements(limit=3):
    rows = _execute(
        "SELECT * FROM announcement WHERE status='publié' ORDER BY date DESC LIMIT ?",
        (limit,)).fetchall()
    return [row_to_announcement(r) for r in rows]
...
def insert_announcement(title, content, status, author):
    _execute(
        "INSERT INTO announcement (id, title, content, date, author, status, is_demo) "
        "VALUES (?,?,?,?,?,?,0)",
...
def update_announcement(ann_id, title, content, status):
    _execute("UPDATE announcement SET title=?, content=?, status=? WHERE id=?",
             (title, content, status, ann_id))
    _commit()


def get_announcement(ann_id):
    return row_to_announcement(
        _execute("SELECT * FROM announcement WHERE id=?", (ann_id,)).fetchone())
    # ============================== DOCUMENTS ==============================

def get_documents(category=None):
    if category and category != "Toutes":
        rows = _execute("SELECT * FROM document WHERE category=? ORDER BY date_added DESC",
                        (category,)).fetchall()
    else:
        rows = _execute("SELECT * FROM document ORDER BY date_added DESC").fetchall()
    return [row_to_document(r) for r in rows]


def get_recent_documents(limit=3):
    rows = _execute("SELECT * FROM document ORDER BY date_added DESC LIMIT ?",
                    (limit,)).fetchall()
    return [row_to_document(r) for r in rows]
...
def get_document(doc_id):
    return row_to_document(
        _execute("SELECT * FROM document WHERE id=?", (doc_id,)).fetchone())


def delete_document(doc_id):
    row = _execute("SELECT name, filename, drive_file_id FROM document WHERE id=?",
                   (doc_id,)).fetchone()
    name = row["name"] if row else ""
    stored_filename = row["filename"] if row else None
    drive_file_id = row["drive_file_id"] if row else None
    _execute("DELETE FROM document WHERE id=?", (doc_id,))
    _commit()
...
def search_documents(q, limit=10):
    ...
    rows = _execute(
        ...
        "ORDER BY date_added DESC LIMIT ?", (like, like, like, limit)).fetchall()
    return [row_to_document(r) for r in rows]
...
def search_members(q, limit=10):
    ...
    rows = _execute(
        ...
        "OR phone LIKE ? ORDER BY first_name LIMIT ?", (like, like, like, like, limit)).fetchall()
    return [row_to_member(r) for r in rows]
...
def insert_notification(title, message, link=None, target_role=None, target_member_id=None):
    _execute(
        ...
        "VALUES (?,?,?,?,?,?,?)",
        (gen_id(), title, message, link, target_role, target_member_id,
         datetime.utcnow().isoformat()))
    _commit()


def get_notifications_for_user(member_id, role=None, limit=100):
    rows = _execute(
        "SELECT * FROM notification WHERE target_member_id = ? "
        "OR target_role = ? OR (target_member_id IS NULL AND target_role IS NULL) "
        "ORDER BY created_at DESC LIMIT ?",
        (member_id, role, limit)).fetchall()
    return [Row(dict(r)) for r in rows]


def count_unread_notifications(member_id, role=None):
    row = _execute(
        "SELECT COUNT(*) c FROM notification n WHERE "
        "(n.target_member_id = ? OR n.target_role = ? OR (n.target_member_id IS NULL AND n.target_role IS NULL)) "
        "AND NOT EXISTS (SELECT 1 FROM notification_read r "
        "WHERE r.notification_id = n.id AND r.member_id = ?)",
        (member_id, role, member_id)).fetchone()
    return row["c"] if row else 0


def mark_notification_read(notification_id, member_id):
    _execute(
        "INSERT INTO notification_read (notification_id, member_id) VALUES (?,?) "
        "ON CONFLICT DO NOTHING", (notification_id, member_id))
    _commit()


def mark_all_notifications_read(member_id, role=None):
    rows = _execute(
        "SELECT id FROM notification WHERE target_member_id = ? OR target_role = ? "
        "OR (target_member_id IS NULL AND target_role IS NULL)",
        (member_id, role)).fetchall()
    for r in rows:
        _execute(
            "INSERT INTO notification_read (notification_id, member_id) VALUES (?,?) "
            "ON CONFLICT DO NOTHING", (r["id"], member_id))
    _commit()
...
def get_audit_logs(limit=200):
    rows = _execute("SELECT * FROM audit_log ORDER BY timestamp DESC LIMIT ?",
                    (limit,)).fetchall()
